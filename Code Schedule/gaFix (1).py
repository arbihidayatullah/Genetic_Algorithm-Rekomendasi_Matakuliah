...
def calculate_fitness(individual):
    ...
    # Check penggunaan ruangan setidaknya satu kali
    used_rooms = set()
    for matkul in individual:
        ruang = kode_ruang[random.choice(list(kode_ruang.keys()))]
        used_rooms.add(ruang)
    if len(used_rooms) < len(kode_ruang):
        penalty += len(kode_ruang) - len(used_rooms)
...
def generate_initial_population():
    ...
    for _ in range(POPULATION_SIZE):
        individual = []
        for _ in range(NUM_MATKUL):
            matkul = random.choice(list(matakuliah.keys()))
            individual.append(matkul)
...
if __name__ == "__main__":
    ...
    if st.button("Proses Penjadwalan"):
        ...
        jadwal_optimal = []
        used_rooms = set()  # Inisialisasi set untuk melacak ruangan yang telah digunakan
        for matkul in best_solution:
            # Pilih ruangan yang belum digunakan sebelumnya, jika tidak tersedia, pilih secara acak dari semua ruangan yang tersedia
            available_rooms = [ruang for ruang in kode_ruang.keys() if ruang not in used_rooms]
            if available_rooms:
                ruang = random.choice(available_rooms)
            else:
                ruang = random.choice(list(kode_ruang.keys()))
            used_rooms.add(ruang)  # Tandai ruangan sebagai digunakan
            jadwal_optimal.append({
                ...
                "Ruang": ruang  # Gunakan ruangan yang telah dipilih
            })
