...
def calculate_fitness(individual):
    ...
    for hari in set([matakuliah[matkul]["hari"] for matkul in individual]):
        if hari:
            ...
            if count_matkul_hari > 3:
                penalty += 2  # Hard Constraint: Mahasiswa mengikuti maksimal 3 mata kuliah dalam 1 hari
    ...
    for jam in set([matakuliah[matkul]["jam"] for matkul in individual]):
        if jam:
            ...
            if count_dosen_jam > 2:
                penalty += 1  # Soft Constraint: Dosen mengajar 2 Matakuliah dalam 1 jam

    # Check for room conflicts
    rooms = set([kode_ruang[random.choice(list(kode_ruang.keys()))] for _ in range(len(individual))])
    if len(rooms) < len(individual):
        penalty += len(individual) - len(rooms)
...
def main():
    ...
    if st.button("Proses Penjadwalan"):
        ...
        # Menampilkan plot dari setiap generasi fitness
