...
def main():
    ...
    for matkul in best_solution:
        ...
        st.write(f"- Matakuliah: {nama_matkul} ({kode_matkul})")
        st.write(f"  Dosen: {dosen_matkul}")
        st.write(f"  Hari: {hari_matkul}")
        st.write(f"  Jam: {jam_matkul}")
        st.write(f"  Ruang: {ruang_matkul}")
